          ...
          fun render(size: Int, state: TideState): Bitmap {
                                    ...
                                    if (arcSweep > 0.5f) {
                                            ...
                                            val tickPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                                                    ...
                                                    tickPaint.color = Color.parseColor("#3D7FD6")
                                                            drawTick(canvas, cx, cy, radius, strokeWidth, -90f, tickPaint)
              ...
              private fun drawTick(
                        ...
                        val rad = Math.toRadians(angleDeg.toDouble())
                                val outer = radius + strokeWidth * 0.9f
                        val inner = radius - strokeWidth * 0.9f
                  ...
                  private fun drawNeedle(canvas: Canvas, cx: Float, cy: Float, length: Float, angleDeg: Float) {
                      ...
                      private fun drawLabels(canvas: Canvas, cx: Float, cy: Float, size: Int, state: TideState) {
                                ...
                                canvas.drawText("pleamar", cx, size * 0.135f, titlePaint)
                                        timePaint.color = blue
                                canvas.drawText("${highLabel}h", cx, size * 0.20f, timePaint)

                                        titlePaint.color = red
                                canvas.drawText("bajamar", cx, size * 0.90f, titlePaint)
                                        timePaint.color = red
                                canvas.drawText("${lowLabel}h", cx, size * 0.955f, timePaint)

                                        sidePaint.color = blue
                                sidePaint.textAlign = Paint.Align.LEFT
                                canvas.drawText("subiendo", size * 0.03f, cy + size * 0.02f, sidePaint)
                                        sidePaint.color = red
                                sidePaint.textAlign = Paint.Align.RIGHT
                                canvas.drawText("bajando", size * 0.97f, cy + size * 0.02f, sidePaint)
                      }
}
